...
describe("useHeroSummary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetSummaryAction.mockImplementation(() => new Promise(() => { }));
  });
  ...
  test('should return success state with data when API call succeeds', async () => {
    ...
    const mockSummaryData = {
      ...
      smartestHero: {
        ...
        slug: 'bruce-wayne',
        alias: 'Batman',
        powers: ['Inteligencia genial', 'Artes marciales', 'Tecnología avanzada'],
  ...
  test('should return error when API call fails', async () => {
    ...
    const mockError = new Error('Failed to fetch summary information');
    ...
    expect(result.current.data).toBeUndefined();
    expect(result.current.isLoading).toBe(false);
    expect(result.current.error?.message).toBe('Failed to fetch summary information');
    expect(mockGetSummaryAction).toHaveBeenCalled();
  });
